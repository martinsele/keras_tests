...
argparser = argparse.ArgumentParser(description='test yolov3 network with coco weights')
argparser.add_argument('-w', '--weights', help='path to weights file')
argparser.add_argument('-i', '--image', help='path to image file')
argparser.add_argument('-m', '--model', help='path to saved model file')
...
def _main_(in_args):
    ...
    if hasattr(in_args, 'model'):
        model_path = in_args.model
    else:
        model_path = None
