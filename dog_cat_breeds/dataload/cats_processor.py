...
def create_folder_structure(data_map: Dict[LabelName, List[str]], data_sizes: DefaultDict[LabelName, int],
    ...
    create_structure(base_folder, data_map.keys())
    ...
    for label, files in data_map.items():
        ...
        copy_cropped_files(train_files, image_folder,
                           os.path.join(base_folder, "cropped_cat", "train", label), annot_folder)
        copy_cropped_files(valid_files, image_folder,
                           os.path.join(base_folder, "cropped_cat", "valid", label), annot_folder)
        copy_cropped_files(test_files, image_folder,
...
def copy_cropped_files(image_list, old_folder, new_folder, annot_folder):
    ...
    for file in image_list:
        ...
        if os.path.exists(old_name):
            save_cropped(file, old_folder, new_folder, annot_folder)
...
if __name__ == "__main__":
    data_folder = "e:/data/dogs_cats/cats_dogs_breed_keggle"
    images_folder = os.path.join(data_folder, "images")
    annotation_folder = os.path.join(data_folder, "annotations", "xmls")

    data_dct, size_dct = read_process_list(os.path.join(data_folder, "annotations", "list.txt"))
    create_folder_structure(data_dct, size_dct, data_folder, images_folder, annotation_folder,
